...
#if defined(NDEBUG)
# error "Bifcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9d61e5d82a687bc905f3225a3400c075defb482cb8e8e04bcadf6e353b1a9a80");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bifcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bifcoin Signed Message:\n";
...
{
    // Bifcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 20000); // Bifcoin: 20k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.5 * 24 * 60 * 60; // Bifcoin: 1.5 days
static const int64 nTargetSpacing = 5 * 60; // Bifcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bifcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bifcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7cf8264bc16d0c3afad1db0b5296d03d6a6c06362a359c254cae5831fb57ad4b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The Last Day Before 2018 12/31/17";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 40 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("046a851116c69b98045bf2efd4f11ed502e173079e3759964010eb42e73f61a633d6502ab0179ac5c15eb38af2881dc15d6fc9b5e661008e3260834692b915a9b8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1514772530;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085081089;

        if (fTestNet)
        {
            block.nTime    = 1514772500;
            block.nNonce   = 385526405;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb50aec7478fe9fc2500819a9043871007be242022d5e0876dc26c4050d2cd253"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc9, 0xb1, 0xde }; // Bifcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BifcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BifcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BifcoinMiner(CWallet *pwallet)
{
    printf("BifcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bifcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BifcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BifcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BifcoinMiner, pwallet));
}
